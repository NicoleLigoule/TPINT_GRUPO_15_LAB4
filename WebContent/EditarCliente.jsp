...
<link rel="stylesheet" href="${pageContext.request.contextPath}/Css/EditarCli.css">
<body>
    <nav class="navbar">
...
<!-- Primer formulario (GET) GUARDAMOS HASTA PROVINCIA -->
			...
			%>
			    ...
			    <input type="hidden" name="localidad" value="<%= request.getParameter("localidad") %>">
			</form>
			...
			<!-- Segundo formulario (POST) USAMOS LO QUE GUARDAMOS ANTES y le sumamos la localidad q cambia segun la provincia elegida -->
				<form action="servletEditarCliente" method="POST">
				    ...
				    <input type="hidden" name="nacionalidad" value="<%= request.getParameter("nacionalidad") %>">
				    <input type="hidden" name="fecha-nacimiento" value="<%= request.getParameter("fecha-nacimiento") %>">
