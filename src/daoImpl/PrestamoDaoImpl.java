...
//        insertDetallePrestamo()
    ...
    public double Porcentajes_p_aprobados() {
        ...
        double porcentaje=0;
        try {
            ResultSet rs = conexion.query("SELECT  (COUNT(CASE WHEN Estado_Pt = 1 THEN 1 END) * 100.0 / COUNT(*)) AS Porcentaje_Prestamos_Activos FROM  Prestamo;");
            if (rs.next()) {
    ...
    public double Rendimientos_p_aprobados() {
        ...
        double resultado=0;
        try {
            ResultSet rs = conexion.query("SELECT   (SUM(DetallesXPrestamo.Importe_C_Interes_Dt) - SUM(Prestamo.Importe_solicitado_Pt)) AS Resultado FROM Prestamo  JOIN DetallesXPrestamo  ON Prestamo.ID_Prestamo_Pt = DetallesXPrestamo.ID_Prestamo_Pt_Dt WHERE    Prestamo.Estado_Pt = 1 AND YEAR(Prestamo.Fecha_Peticion_Pt) = YEAR(CURDATE());");
            if (rs.next()) {
